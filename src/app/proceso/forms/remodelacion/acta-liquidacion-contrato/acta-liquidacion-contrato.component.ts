...
@Component({
  selector: 'app-acta-liquidacion-contrato',
  templateUrl: './acta-liquidacion-contrato.component.html',
  styleUrls: ['./acta-liquidacion-contrato.component.css']
})
export class actaLiquidacionContratoComponent extends ComunTaskArchivosComponent implements OnInit, OnDestroy {
  ...
  onUpload(e) {
    ...
    if(file){
      ...
      this.solicitud.nombreActaLiquidacionC = this.nameDocUp;
    }
    const filePath = `docs/${this.solicitud.id}/ActaLiquidacionContrato_${id}`;
    ...
    this.swal.showQuestionMessage('').then( resp => {
      if(resp.value){
        ...
        this.urlDoc
            .pipe(takeUntil(this.ngUnsubscribe))
            .subscribe(url => {
              this.solicitud.urlActaLiquidacionC = url;
              this.solicitud.nombreActaLiquidacionC = this.nameDocUp;
