...
@Component({
  selector: 'app-emitir-resolucion',
  templateUrl: './emitir-resolucion.component.html',
  styleUrls: ['./emitir-resolucion.component.css']
})
export class emitirResolucionComponent extends ComunTaskArchivosComponent implements OnInit, OnDestroy {
  ...
  onUpload(e) {
    ...
    if(file){
      ...
      this.solicitud.nombreResolucion = this.nameDocUp;
    }
    const filePath = `docs/${this.solicitud.id}/Resolucion_${id}`;
    ...
    this.swal.showQuestionMessage('').then( resp => {
      if(resp.value){
        ...
        this.urlDoc
            .pipe(takeUntil(this.ngUnsubscribe))
            .subscribe(url => {
              this.solicitud.urlResolucion = url;
              this.solicitud.nombreResolucion = this.nameDocUp;
