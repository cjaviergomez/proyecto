...
export class entregaReformaComponent extends ComunTaskArchivosComponent implements OnInit, OnDestroy {
  ...
  onUpload(e) {
    ...
    if(file){
      ...
      this.solicitud.nombreEntregaR = this.nameDocUp;
    }
    const filePath = `docs/${this.solicitud.id}/EntregaReforma_${id}`;
    ...
    this.swal.showQuestionMessage('').then( resp => {
      if(resp.value){
        ...
        this.urlDoc
            .pipe(takeUntil(this.ngUnsubscribe))
            .subscribe(url => {
              this.solicitud.urlEntregaR = url;
              this.solicitud.nombreEntregaR = this.nameDocUp;
